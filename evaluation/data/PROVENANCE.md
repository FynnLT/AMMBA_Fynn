...
**Units.** Values in `Load`, `PV` and `BESS` are read as **kW**; energy per
period is `value × 0.25 h`. Grounds: the sheet headers use kW
(`Capacity (kW)`, `Power Buy (kW)`), the resulting mean household load is
1.02 kW, and reading them as kWh would put a household at 98 kWh/day. **To be
confirmed against the *Data in Brief* article before Chapter 5 states a
unit.**
...
| Supply/demand ratio range | 0.0005 … 2.1184 |
